...
| **一键更新** | 可在启动后检查 GitHub Releases 并显示完整更新说明；点击一次即可下载适配当前架构的 MSI、校验发布页 SHA-256、请求 Windows UAC 授权、覆盖升级并自动重启。 |
| **隐私边界清晰** | 无遥测；更新检查和用户主动触发的安装包下载只访问公开 GitHub Release，历史与日志仍只保存在 `%APPDATA%`。 |
...
程序平时以普通用户权限运行。开启“启动时检查更新”后，会匿名请求已配置公开 GitHub 仓库的 latest
Release 接口，用于取得界面里显示的版本号、发布日期和更新说明。只有用户主动点击“一键更新”时，才会
下载对应架构的 MSI 和 `SHA256SUMS.txt`，校验成功后请求 UAC 授权，由 Windows Installer 完成升级。
这个选项可以关闭，并且不会上传文件名、历史记录、预设、日志、设备标识或使用数据。
