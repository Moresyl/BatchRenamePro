...
<!--
    截图放这里——这是 README 目前唯一还缺的东西，而对桌面程序来说它比上面任何一段文字都管用。
    窗口最大化，按 1320x860 截三张，存到 docs/images/，用 <div align="center"> 引用：

      1. rename.png    叠了几条规则、预览区显示差异高亮的改名页
      2. presets.png   预设页
      3. dark.png      同一个改名页的另一种主题，用来展示配色

    每张控制在 400 KB 以内，手机上也能秒开。
-->
...
| **编号** | 作为前缀或后缀的连续编号——起始值、步长、补零位数、分组大小、数字或字母序列。 |
| **插入** | 在开头、结尾或指定字符位置插入文本。 |
